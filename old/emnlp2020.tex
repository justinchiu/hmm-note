...
Unfortunately, this clean separation makes HMMs difficult to fit to large datasets in modern NLP, 
...
scaling HMMs to language modelings datasets, taking ideas from recent approaches to neural modeling.
...
In recent years, most state-of-the-art systems in NLP have moved away from utilizing explicit hidden states,
...
In this work, we revisit HMMs for NLP, positing that competitive performance requires very large HMM (VL-HMMs). To scale to large state-spaces, we need  shared parameterizations, efficient inference, and 
...
Experiments employ VL-HMMs on two language modeling datasets and a supervised sequence tagging task.
...
neural networks with moderate success. \citet{buys2018hmm}
develop an approach to relax the independence and modeling assumptions to resemble a recurrent neural network. However, their results either report HMM performance much worse than
an n-gram model for language modeling or require altering the fundametal probabilistic structure of the model. 
\citet{krakovna2016hmm} utilize model combination with a recurrent neural network to connect both approaches in a 20 state model. We demonstrate how to scale to orders of magnitude more states and show performance surpassing n-gram models.
...
Inference in an HMM can be sped up to be logarithmic in sequence length
on a parallel machine via the prefix-sum algorithm \cite{ladner1980prefix}.
...
\subsection{Factored Neural Parameterization}
...
The scalar parameterization of a very large HMM grows quadratically due to the transition matrix parameters $\psi \in \mathbb{R}^{|\mcZ|\times|\mcZ|}$.
We use the intuition that introducing a parameter for each state pair is unnecessary,
and instead employ state embeddings which allow the number of parameters to scale linearly with the size of the state space.
...
\begin{equation}
\begin{aligned}
\phi = E_x f_\phi(E_\textrm{emit})^\top \;\; 
\psi = E_\textrm{out} f_\psi(E_\textrm{in})^\top
\end{aligned}
\end{equation}
where $\phi \in \mathbb{R}^{|X|\times|Z|}$ and
$\psi \in \mathbb{R}^{|Z|\times|Z|}$,
and $f$ is an MLP. \footnote{For the MLP we  use the following neural network
\begin{equation}
\label{eqn:res}
\begin{aligned}
...
\end{aligned}
\end{equation}
where $D$ has the dimensions of $E$
...
We further parameterize the embeddings $E_{in},E_{out},E_{emit}$
by factoring them into partition and state embeddings.
We introduce $m$ partition embeddings $e_p\in\mathbb{R}^{h/2}$,
and construct the partition embeddings
$E_\pi = [e_{\pi(1)}, e_{\pi(2)}, \ldots, e_{\pi(|\mcZ|)}] \in \mathbb{R}^{|\mcZ|\times h/2}$.
Given state embeddings $E_\mcZ \in \mathbb{R}^{|\mcZ| \times h/2}$, we define
%\begin{equation}
%\label{eqn:emb}
\begin{alignat}{4}
&E_\textrm{in} &&=&& f_\textrm{in}&([E_\pi, E_\mcZ]) \\
&E_\textrm{out} &&=&& f_\textrm{out}&([E_\pi, E_\mcZ]) \\
&E_\textrm{emit} &&=&& f_\textrm{emit}&([E_\pi, E_\mcZ])
\end{alignat}
%\end{equation}
This parameterization further reduces the number of model parameters
to $O\left(h^2 + \frac{h}{2}|\mcZ| + \frac{h}{2}m + h|\mcX|\right)$.

Given the distributional parameters, we can then perform marginal inference separately. 
...
For training, we can compute the emission and transition distributions once per batch.
This contrasts with RNNs and other auto-regressive neural networks,
...
For instance, if we know that the probability of emitting a word $x_t$ from a state $z_t$ is 0,
...
\caption{
\label{fig:algo}
Pseudocode for computing the likelihood of a corpus with state dropout
}
\end{algorithm*}
...
We propose a form of HMM state dropout that removes states from use entirely for a batch.
...
We use Brown clusters \citep{brown1992} to construct the state and word partitions.
...
The merge at each iteration attempts to merge the two states that results in the best likelihood.

Importantly, every word is emit by only a single state, yielding the word partition $\rho$
where the number of partitions $m$ equal to the number of Brown clusters.
...
We use the same partitions, emission constraints, and dropout strategy as language modeling,
...
This method limits interaction between tags.
...
%and for \texttt{wikitext2} in Table \ref{tbl:wt2-ppl}.
...
(need longer-term dependency experiment, measure trend as num states is increased).
...
We analyze the performance of a 16k state HMM,
keeping the total number of states fixed
...
We also experiment with the uniform constraints as described in Sec.~\ref{sec:experiments}.
...
We additionally ablate the factored state embeddings (add to table),
and find that the performance of the factored embeddings drops
when the number of clusters is too small.
(Makes sense that )
(Indicates number of parameters not necessarily the best comparison)
...
for a 16k state HMM with 0.5 state dropout, and ablate
