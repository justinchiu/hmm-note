...
We also consider unstructured dropout
which samples elements of masks iid $b_{z_0}, b_{z_tz_{t-1}}\overset{iid}{\sim} \Bern(p)$
...
since there is variance in the number of masked states that can emit a word.
(Fix wording)

We find that state dropout has similar regularization effect as unstructured dropout,
but additionally yields computational benefits.
...
\paragraph{States per word ($k$) ablation}
