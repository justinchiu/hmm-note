...
In this work, we revisit HMMs for NLP, positing that competitive performance requires very large HMM (VL-HMMs). To scale to large state-spaces, we need  appropriate parameterizations, efficient inference, and 
...
We use the intuition that introducing a parameter to model between each state pair is unnecessary,
...
To encourage redundancy and distribute state usage, we introduce dropout to the model. While we could use dropout on the parameterization alone, 

variant of dropout called state dropout that lessens
