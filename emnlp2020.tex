...
These developments in language modeling taught us three lessons.
Parameterization is important, as the feedforward neural network 
of \citet{bengio2003nlm} had the same independence assumptions as an n-gram model
but achieved better performance.
In addition to parameterization, a large amount of parameters
or even overparameterization can lead to performance improvements \citep{zaremba2014lstm}
(cite other overparam papers).
Finally, our models need to be able to capture long term dependencies.

In this work, we apply the first two of the lessons learned from improving fully observed
language models to HMMs, while leaving the third to future work.
...
we found the HMM to outperform n-gram models and perform comparably to
neural network counterparts (cite).
