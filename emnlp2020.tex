...
providing one of the simplest models of sequential data that posits
a generative process involving latent variables. 
...
HMMs were not considered until recently \citep{krakovna2016hmm,dedieu2019learning}.
...
We learned three lessons from language modeling progress.
The first lesson is that parameterization is important for learning.
...
The second lesson is that overparameterization is helpful as well.
...
The third lesson is that regularization is necessary with small
...
In this work, we apply the lessons learned from improving fully observed
...
Both of these works used latent variables with relatively small cardinalities,
...
tractable in extremely large state spaces.
...
and focus on scaling the simplest incarnation of the HMM.

\section{Background}
We focus on language modeling,
where we learn a distribution over sequences of tokens
at the word level $\bx = \langle x_0, \ldots, x_T \rangle$, with each token $x_t$
an element of the finite vocabulary $\mcX$.

\subsection{Hidden Markov Models}
Hidden Markov Models (HMMs) specify a joint distribution over 
the observed words $\bx$ and discrete latent states $\bz = \langle z_0, \ldots, z_T \rangle$,
