...
In recent years, most state-of-the-art systems in NLP have moved away utilizing explicit hidden states. (Something about sequence modleing)
particularly for structured models. In the area of language models modeled the observed words using n-gram models, 
...
This progress has led to the common wisdom that latent-variable models are not competitive with fully observed models.


In this work, we revisit the question of developing HMMs for natural language processing, in particular looking at the tasks of language modeling and sequence tagging. We approach this question by taking direct lessons from the success of deep neural models for NLP tasks. In particular we make three observations: (1) The right parameterization is critically important for representation learning, e.g. a feedforward model \citet{bengio2003nlm}
can have the same probabilistic structure as an n-gram model while performing significantly better;
(2) overparameterization seems to be critical for finding better optima, e.g. large LSTMs \citet{zaremba2014lstm} show marked improvements in performance; (3) regularization choices are necessary to handle reconcile (different) parameterizations, e.g. experiments by \citet{merity2017awdlstm} outline a variety of training choices.

These observations motivate a new approach to classical  HMMs. We develop a neural parameterization for HMMs that extends 
them to comparable size and structure of deep learning models, while allowing us to lazily instantiate distributions. We also develop an  sparsity constraint that allows us to utilize very large HMMs, significantly beyond standard efficiency limits. Finally we incorporate a variant of dropout that both improves accuracy and reduces the computational overhead by an order of magnitude during training. 
...
It also performs comparably to NN counterparts while maintaining ... 
...
via this process.
This is an avenue for future work, as we focus on fixed-size state spaces
for simplicity.

% Refer to model combination paper, such as kj
...
an element of the finite vocabulary $\mcX$. Hidden Markov models (HMMs) specify a joint distribution over 
observed tokens $\bx$ and discrete latent states $\bz = \langle z_0, \ldots, z_T \rangle$,
with each $z_t$ from finite set $\mcZ$.
The model is defined by the following generative process (shown in figure~\ref{}):
...
\label{param}
\begin{aligned}
p(z_1 | z_0) &\propto e^{\phi_{z_1}} \ p(z_t \mid z_{t-1}) &\propto e^{\phi_{z_tz_{t-1}}}\\
p(x_t \mid z_t) &\propto e^{\phi_{x_tz_t}}
\end{aligned}
\end{equation}
where each $\phi_{z_0},\phi_{z_tz_{t-1}},\phi_{x_tz_t} \in \mathbb{R}$. We distinguish two types of  parameterizations: \textit{scalar} and \textit{neural}.

(Matrices)

A scalar parameterization simply uses $\theta = \phi$ to fit one model parameter for each distributional parameter. This results in $O(|\mcZ|^2 + |\mcZ||\mcX|)$ model parameters,
since the transition and emission matrices must be explicitly represented. This parameterization can lead to either in over or under parameterization for most NLP tasks. In contrast a neural parameterization uses $\theta$ as the parameters of a neural network that generates $\phi$ for the distribution. These may be customized for the task through embedding or other layers, and separate the parameterization size from the hidden state size. 



%1. Something about inference $p(z | x)$ for training and for filtering. 

For sequence modeling with an HMM,
we must marginalize over the latent states $p(\bx) = \sum_{\bz}p(\bx,\bz)$
...
2. Compare complexity of forward in LSTM / HMM.

%matmul for lstm vs matmul for HMM. 
%cost of computing transition matrix is "amortized" over examples)

\begin{figure}[t]
% purpose: show that both models we consider are literally HMMs
\centering
\begin{subfigure}[t]{0.4\textwidth}
\centering
\begin{tikzpicture}[]
\node[obs]    (x0) {$x_0$} ;
\node[obs]    (x1) [right=1.25cm of x0] {$x_1$} ;
\node[obs]    (x2) [right=1.25cm of x1] {$x_2$} ;
\node[latent] (z0) [below = 0.75cm of x0] {$z_0$};
\node[latent] (z1) [below = 0.75cm of x1] {$z_1$};
\node[latent] (z2) [below = 0.75cm of x2] {$z_2$};
...
\node[obs]    (x1) [right=1.25cm of x0] {$x_1$} ;
\node[obs]    (x2) [right=1.25cm of x1] {$x_2$} ;
\node[obs]    (y0) [right=0.29cm of x0] {$y_0$} ;
\node[obs]    (y1) [right=0.29cm of x1] {$y_1$} ;
\node[obs]    (y2) [right=0.29cm of x2] {$y_2$} ;
\node[latent] (z0) [below right = 0.75cm and 0.08cm of x0] {$z_0$};
\node[latent] (z1) [below right = 0.75cm and 0.08cm of x1] {$z_1$};
\node[latent] (z2) [below right = 0.75cm and 0.08cm of x2] {$z_2$};
\edge {z0} {x0};
\edge {z1} {x1};
\edge {z2} {x2};
\edge {z0} {y0};
...
HMMs cleanly separate out latent memory from the generative process of language. This separation is makes them an important model for isolating observed properties and performing inference. However this separation also means they require a much larger hidden state space to capture the underlying properties of language. 
...
%However, in order to increase the capacity of HMMs, we would like to consider large $\mcZ$.
%We present two techniques to manage the computational complexity of marginalization in HMMs.  


\subsection{Posterior State Constraints}
%We introduce a model constraint that allows us to
%efficiently perform conditional inference in large %state spaces.

Posterior inference for general HMMs is quadratic in the  state space $|\mcZ|$. In practice, this limits the number of plausible states to the order of 100s, which prevents models from capturing the full history. However, we can limit the complexity in special cases. In particular, if we know that the posterior probability of a state $z_t$ is 0, i.e. $p(z_t | x_t) = 0$  then we can ignore it during 
posterior inference. 



%The issue with large state spaces is that marginalization is quadratic in $|\mcZ|$.
%At every timestep, the HMM must consider transitions from all previous states to every possible current state.
%In order to scale to a large number of states, we decouple the number of legal states for each timestep from the total number of states.

Inspired by cloned HMMs \citep{dedieu2019learning},
we add explicit posterior constraints to ensure that this properties holds at every time step. Specifically, we ensure that conditioned on an observed token $x$, a fixed number of states $z$ have $p(z | x) > 0$, i.e. non-zero chance of having emitted $x$, and we call this set $\mcC_x \subset \mcZ$.
%\begin{equation}
...
due to convenience of implementation. (GPU)
...
Actually give the full model. 

%may take as few as $O(H(|\mcZ| + |\mcX|))$ parameters,
...
Brown clusters , LM , part-of-speech. 
...
We introduce the following change to the HMM language model's generative process:
...
In Figure~\ref{fig:hmm}, we see that this model remains in the family of HMMs.
The same parameterization, word emission constraints, and state dropout are applied to this model,
while the tag emission distribution is unconstrained.
