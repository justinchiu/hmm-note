...
a regularized neural parameterization and efficient exact inference, we can build models that are competitive with RNNs. We propose three techniques for scaling these models: factored neural parameterization, induced block transitions, and state dropout.
...
and $E_{\textrm{out}},E_{\textrm{in}},E_\textrm{emit}
\in \mathbb{R}^{|\mcZ|\times h}$ be embeddings for leaving, entering, and emitting from a state respectively. 
The distributional parameters are given by,
\begin{equation}
\begin{aligned}
\phi = E_x f_\phi(E_\textrm{emit})^\top \;\; 
\psi = E_\textrm{in} f_\psi(E_\textrm{out})^\top
\end{aligned}
\end{equation}
where $\phi \in \mathbb{R}^{|X|\times|Z|}$ and $\psi \in \mathbb{R}^{|Z|\times|Z|}$, and $f$ is an MLP. \footnote{For the MLP we  use the following neural network
\begin{equation}
\label{eqn:res}
\begin{aligned}
...
This contrasts with RNNs and other auto-regressive neural networks, which must compute emission probabilities for every observation. 
...
only a fixed number of states $z$ have $p(x \mid z) > 0$.
...
%State dropout is  implemented by adding another constraint to the emission matrix.
...
Performing marginal inference with state dropout allows us to skip states that are removed. To ensure the block structure of the previous section
...
that a 32k state HMM is able to outperform the n-gram models (with cache)
as reported by \citet{mikolov2012rnn}, achieving a test perplexity of
115.8 versus 125.7 respectively.
