...
We distinguish between two types of parameterizations: \textit{scalar} and \textit{neural}.
The distinction lies in the model parameters $\theta$.
...
In the neural parameterization, we have $\phi,\psi = f(\theta)$,
...
\end{aligned}
\end{equation}
This gives a serial complexity of $O(Tk^2)$.
...
and assume the vocabulary remains identical at test time (OOV words mapped to unk).

\noindent\textbf{State Dropout}
We use a dropout rate of $\lambda = 0.5$ at training time. 
For each block of size $|\mcX_m|$, we sample $|\mcX_m|/2$ states to use in that block each batch.
At test time we use the full state distribution without dropout.
...
VLNHMM $|\mcZ|=2^{15}$ & 7.7M & 125.0      & 115.8  \\
\midrule
\texttt{WikiText}\\
\midrule
...
VLNHMM $|\mcZ|=2^{15}$  & 13.7M   & 169.0 & 158.2\\
\bottomrule
\end{tabular}
...
TODO: rerun with IND embeddings.
}
\end{figure}
...
TODO: RERUN DROPOUT ABLATION WITH IND EMBEDDINGS.
}
\end{table}
