...
%\begin{tikzpicture}[mymatrixenv]
    ...
    %\node[rectangle,draw] (Epi) at ($(m-3-2.south) + (0, -2)$) {$E_\pi$};
    %\node[rectangle,draw] (Ez) at ($(m-3-6.south) + (0, -2)$) {$E_z$};
    \node[rectangle,draw] (p1) at ($(m-3-5.south) + (0, 1)$) {MLP$(\mathbf{E}_\pi,\mathbf{E}_z)$};
    \node[rectangle,draw] (Ex) at ($(m-3-9.south) + (0,1)$) {$\mathbb{E}_x$};
    \coordinate[] (p2) at ($(m-3-7.south) + (0, 0.2)$);

    %\draw[-, thick] (Epi) to (p1);
    %\draw[-, thick] (Ez) to (p1);
    \draw[-, thick] (p1) to (p2);
    \draw[-, thick] (Ex) to (p2);
