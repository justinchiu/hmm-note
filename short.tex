...
and have structure complementary to modern neural models.
...
\begin{equation}
\label{eqn:mat_ev}
p(\bx) = \mathbf{1}^\top\mathbf{F}_1 \bF_2 \cdots \bF_{T-1}\bF_{T}\mathbf{1}
\end{equation}
...
\end{aligned}
\end{equation}
To make the computational gains more explicit, we also present the above in matrix form.
...
Additionally, if $z_{t-1}$ is not contained in the corresponding emission block corresponding to $x_{t-1}$ ($z_{t-1}\notin\mcZ_{x_{t-1}}$) then it has zero probability of occurring. We can therefore ignore any outgoing transitions from that state.
Combining these two observations yields the following, where we only consider transitions from $\mcZ_{x_{t-1}}$ to $\mcZ_{x_t}$:
\begin{equation}
\label{eqn:sparse_factors}
F_{tij}^b = \begin{cases}
...
$p(\bx) = \mathbf{1}^\top\bF_1^b\bF_2^b\cdots\bF_T^b\mathbf{1}$
from $|\mcZ|^2$ to $k^2$,
...
Given the set of of all dropped states $\mcD$,
the emission and transmission probabilities are
\begin{equation}
\label{eqn:state_dropout}
\begin{aligned}
p(z_t \mid z_{t-1}) &\propto 1(z_t,z_{t-1}\in \mcD^C )e^{\psi_{z_{t-1}z_t}}\\
p(x_t \mid z_t) &\propto 1(z_t \in \mcZ_{x_t} \cap \mcD^C)e^{\phi_{z_tx_t}}
\end{aligned}
\end{equation}
This allows us to introduce further sparsity into the computation of the likelihood.
Let $\mathbf{F}_t^d$ be a sparse factor after applying both the block emission constraints
and state dropout, such that
\begin{equation}
\label{eqn:dropout_factors}
F_{tij}^d \propto \begin{cases}
F_{tij}, & i\in\mcZ_{x_{t-1}}\cap \mcD^C \wedge j\in\mcZ_{x_t} \cap\mathcal{D}^C\\
0, & \textrm{otherwise}
\end{cases}
\end{equation}
The resulting sparsity pattern in the likelihood computation is shown in Fig.~\ref{fig:trellis}.
...
The marginal improvement as the number of states decreases, implying that the current approach may have limitations in scaling to even larger state spaces.
...
Ablations on \texttt{PTB} ($\lambda =0.5$ and $M=128$). Size is the number of parameters, while train and val give the corresponding perplexities.
Time is ms per eval batch (Run on RTX 2080).
}
\end{table}
