...
while maintaining a efficient exact inference, a compact parameterization,
...
Additionally, we introduce three techniques to scale HMMs:
...
We observe that when we remove the block embeddings and directly parameterize
the state representations performance the model is faster
due to not needing to recompute state representations.
This contrast is even more obvious when removing neural components altogether,
