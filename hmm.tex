...
For each $t \in [T]$
\begin{enumerate}
\item Choose $z_t \mid z_{t-1} \sim \Cat(\cdot)$
\item Choose $x_t \mid z_t \sim \Cat(\cdot)$
\end{enumerate}
with $p(z_1 \mid z_0) = p(z_1)$.

This gives the following joint distribution:
\begin{align*}
\log p_\theta(x_{1:T}, z_{1:T}) &= \sum_t \log p_\theta(x_t, z_t \mid z_{t-1})\\
&= \sum_t \log p_\theta(x_t\mid z_t) + \log p_\theta(z_t \mid z_{t-1})
\end{align*}

\section{Parameter estimation}
We maximize the evidence $\log p(x_{1:T} = \log \sum_{z_{1:T}} p(x_{1:T})$.
